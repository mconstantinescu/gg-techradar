...
- This record is **Product/Program scope** and would bind all teams
  ...
  platform.
- Currently **Proposed** — feedback from service owners requested before
  formal acceptance.
...
We propose adopting an **API-First** development workflow:
...
- **Breaking-change detection** runs in CI against the previous published
  spec version
...
- **Mitigation**: Provide OpenAPI starter templates; integrate Spectral
  into PR checks; schedule quarterly API review sessions.


**Update March 2026:** After successful adoption across all API teams, the API-first approach with OpenAPI specifications is now standard. All public and internal APIs must have an OpenAPI 3.1 specification published to the API catalogue before implementation begins.
